...
<br></br>
<br></br>
<h4>THE DRAW, SHUFFLE AND CUT</h4>
From a shuffled pack face down, each player cuts a card, leaving at least four cards at either end of the pack. If both players cut cards of the same rank, each draws again. The player with the lower card deals the first hand. Thereafter, the turn to deal alternates between the two players, except that the loser of the game deals first if another game is played. The dealer has the right to shuffle last, and presents the cards to the non-dealer for the cut prior to the deal. (In some games, there is no cut at this time.)
<br></br>
<br></br>
<h4>THE DEAL</h4>
...
The board is placed to one side between the two players, and each player takes two pegs of the same color. (The pegs are placed in the game holes until the game begins.) Each time a player scores, they advance a peg along a row on their side of the board, counting one hole per point. Two pegs are used, and the rearmost peg jumps over the first peg to show the first increment in score. After another increase in score, the peg behind jumps over the peg in front to the appropriate hole to show the player's new score, and so on (see diagram next page). The custom is to "go down" (away from the game holes) on the outer rows and "come up" on the inner rows. A game of 61 is "once around" and a game of 121 is "twice around." As noted previously, continuous line Cribbage boards are available.
