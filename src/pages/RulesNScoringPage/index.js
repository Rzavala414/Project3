...
export default class RulesNScoringPage extends Component {
    render() {
        return (
            <div>
                <h1>This is RulesNScoringPage</h1>
                <p className="rules">
                    Rules
    Main article: Rules of cribbage
    ...
    Deal
    The players cut for first deal, and the person who cuts the lowest card deals. The dealer shuffles and deals five or six cards to each player, depending on the number of players. For two players, each is dealt six cards; for three or four players, each is dealt five cards. In the case of three players, a single card is dealt face down in the centre of the table to start the crib. Once the cards have been dealt, each player chooses four cards to retain, then discards the other one or two face-down to form the "crib" (also called the box), which will be used later by the dealer.[7] At this point, each player's hand and the crib will contain exactly four cards. The player on the dealer's left cuts the remaining deck, and the dealer reveals the top card, called the "starter" or the "cut".[8] If this card is a jack, the dealer scores two points for "his heels" or "his nibs".
    ...
    Rules sourced from https://en.wikipedia.org/wiki/Cribbage
                </p>
